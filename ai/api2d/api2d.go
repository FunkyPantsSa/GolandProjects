...
func Api2d(token string, question string) (answer string) {
	...
	headers := map[string]string{
		...
		//"Authorization": "Bearer fk196722-qZJvA7TX ", // 把 fkxxxxx 替换成你自己的 Forward Key，注意前面的 Bearer 要保留，并且和 Key 中间有一个空格。
	}
	...
	//gjson.Get(answer, "choices.0.message.content").ForEach(func(key, value gjson.Result) bool {
	//	data = value.String()
	//	return true
	//})
	...
	var data2 string
	//content2 := MyResponse{}
	//if err := json.Unmarshal([]byte(body), &content2); err != nil {
	//	panic(err)
	//}
	gjson.Get(answer, "choices.0.message.content").ForEach(func(key, value gjson.Result) bool {
...
//
//	//jsonToData(body)
//	//println(jsonToData(api2d(input())))
//	for {
//		fmt.Printf("我：")
//		println("AI:", jsonToData(api2d(input.Input())))
//	}
//
//}
